...
namespace SearchApi.Web.Test.People
{
    public class PeopleControllerTest
    {
        ...
        {
            var result =
                (AcceptedResult) this._sut.Search(null, new PersonSearchRequest("firstName", "lastName", null, new List<PersonalIdentifier>(), new List<Address>(), new List<SearchApiPhoneNumber>(), new List<SearchApiName>())).Result;
        ...
        public void With_valid_payload_and_id_should_return_created()
        {
            ...
            var result =
                (AcceptedResult)this._sut.Search($"{expectedId}", new PersonSearchRequest("firstName", "lastName", null, new List<PersonalIdentifier>(), new List<Address>(), new List<SearchApiPhoneNumber>(), new List<SearchApiName>())).Result;
            Assert.IsInstanceOf<PersonSearchResponse>(result.Value);
